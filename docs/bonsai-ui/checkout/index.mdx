...
type Config = {
  ...
  /* A custom message displayed on the order success page. `Shipping` data is provided as an argument
   * to the function, so you can get access to the shipping information. If `null` is returned, the
   * default message will be displayed.
   */
  customOrderSuccessMessage?: (shipping: Shipping) => string | null;
